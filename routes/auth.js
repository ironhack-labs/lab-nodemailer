...
router.get('/confirm/:confirmationCode', (req, res, next) => {
  ...
  .then(user => {
      if (user) {
        ...
        user.save()
          .then((user) => {
            res.render('auth/confirmation', { message: 'Your account has been activated, login now!' })
          })
          .catch((error) => next)
      } else {
        res.render('/', { error: { validation: { message: 'An error has occurred. Try to sign-up again!'}}})
      }
    })
})
