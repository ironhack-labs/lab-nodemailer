...
router.post('/signup', async (req, res, next) => {
  ...
  if (username === '' || password === '' || email === '') {
    res.render('auth/signup', { message: 'Write username, email and password please' })
    return
  }
  ...
  const info = await transporter.sendMail({
    from: `El Cris <${process.env.EMAIL}>`,
    to: email,
    subject: 'Confirm your account please',
    html: `<a href="http://localhost:3000/auth/confirm/${confirmationCode}">
    <p>Thanks to join our communit, please confirm your account clicking on the following link</p>
    <h1>Click here to confirm your account</h1></a>`
  })
